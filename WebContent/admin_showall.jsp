			...
			if(needNum==0){
				...
				for(int p=1;p<=pageNum;p++){
					...
					for(int i=0;i<pageNeedNum;i++){
						...
						out.print("<td><a style='text-decoration: none' href='admin_needinfo.jsp?num="+v.get(i+p*pageMax-pageMax).getNum()+"'>详情</td>");
						out.print("<td><a style='text-decoration: none' num='"+v.get(i+p*pageMax-pageMax).getNum()+"' st='已通过' class='pass' href='#'>通过</td>");	
						out.print("<td><a style='text-decoration: none' num='"+v.get(i+p*pageMax-pageMax).getNum()+"' st='被退回' class='reject' href='#'>退回</td>");
						out.print("</tr>");
					}
					out.println("</table>");
					out.println("</div>");
...
$(".pass").click(
	function(){
		$.post("UpdateNeedServlet",
				getParams($(this).attr("num"),$(this).attr("st")),
